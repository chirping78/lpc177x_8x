 ...
 *  (And one range for GPIO peripheral but assigned in SRAM ranges: 0x20098000 - 0x200980BF)
 */
 ...
 * 	+ 0x20000000 - 0x20007FFF: for SRAM
 *  + 0x20098000 - 0x200980BF: for GPIO
 */
#define VAR_ADDRESS		0x20000000
#define VAR_BIT			30 //Bit 0~31

/* Peripheral address
 * should be in range: 0x40000000 - 0x40100000
 */
#define PERI_ADDRESS	0x40088004 // SSP0CR1
#define PERI_BIT		2 //bit 2 - Master mode select
...
int c_entry(void) { /* Main Program */
	...
	_DBG("Use bit-band function to get value at bit ");_DBD(VAR_BIT);_DBG(": ");_DBG_("");
	_DBH32(temp); _DBG_("");
	...
	/* re-read temp value */
	temp = (*(volatile uint32_t *)(VAR_ADDRESS));
	_DBG("Value after clear bit ");_DBD(VAR_BIT);_DBG(" value by using bit-band function: ");_DBG_("");
	_DBH32(temp); _DBG_("");
	...
	/* re-read temp value */
	temp = (*(volatile uint32_t *)(VAR_ADDRESS));
	_DBG("Value after set bit ");_DBD(VAR_BIT);_DBG(" value by using bit-band function: ");_DBG_("");
	_DBH32(temp); _DBG_("");
	_DBG_("");
	...
	(*(volatile uint32_t *)(PERI_ADDRESS)) =(1<<2);
	...
	_DBG("Use bit-band function to get value at bit ");_DBD(PERI_BIT);_DBG(": ");_DBG_(""); _DBH32(temp); _DBG_("");

	/* clear bit */
	BITBAND_PERI_ClearBit(PERI_ADDRESS,PERI_BIT);
	/* re-read temp value */
	temp = (*(volatile uint32_t *)(PERI_ADDRESS));
	_DBG("Peripheral register after clear bit ");_DBD(PERI_BIT);_DBG(" value by using bit-band function: ");_DBG_("");
	_DBH32(temp); _DBG_("");
	...
	/* re-read temp value */
	temp = (*(volatile uint32_t *)(PERI_ADDRESS));
	_DBG("Peripheral register after set bit ");_DBD(PERI_BIT);_DBG(" value by using bit-band function: ");_DBG_("");
	_DBH32(temp); _DBG_("");
