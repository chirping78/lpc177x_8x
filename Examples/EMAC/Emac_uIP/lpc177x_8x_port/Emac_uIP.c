...
void c_entry(void)
{
    ...
    _DBG_("uIP porting on LPC17xx (WebServer application)");
#elif defined(WEB_CLIENT)
    _DBG_("uIP porting on LPC17xx (WebClient application)");
#elif defined (SMTP_APP)
    _DBG_("uIP porting on LPC17xx (SMTP application)");
#elif defined(TELNET_APP)
    _DBG_("uIP porting on LPC17xx (TELNET application)");    
#else
