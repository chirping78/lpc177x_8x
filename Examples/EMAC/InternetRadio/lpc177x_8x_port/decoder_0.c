/**********************************************************************
* $Id$      decoder.c     2011-06-02
*//**
* @file     decoder.c
* @brief    This example describes how to use I2S to play audio data 
*               on LPC177x_8x
* @version  1.0
* @date     02. June. 2011
...
/** Max buffer length */
#define IN_FRAME_MIN_SIZE               (2*MAINBUF_SIZE)
...
#define DECODER_OUT_BUFF_SIZE           (OUT_FRAME_MAX_SIZE)

#define DECODER_IN_BUFF_START_ADDR      (SDRAM_BASE_ADDR)
#define DECODER_OUT_BUFF_START_ADDR     (DECODER_IN_BUFF_START_ADDR + DECODER_IN_BUFF_SIZE)
#define AUDIO_BUFF_START_ADDR           (DECODER_OUT_BUFF_START_ADDR + DECODER_OUT_BUFF_SIZE)
...
uint32_t AudioBuf_Write(AUDIO_BUFFER *buff, uint8_t* data, uint32_t size);
...
/*********************************************************************//**
 * @brief       Write data to audio buffer
...
uint32_t AudioBuf_Write(AUDIO_BUFFER *buff, uint8_t* data, uint32_t size)
{
    ...
    {
        memcpy(&buff->buffer[buff->tail], data, size);       
...
#if DEBUG_DATA_WRITE        
    {
        ...
        {  
            _DBG("Write ");_DBD32(((uint64_t)wsamples_cnt*CLOCK_CONF_SECOND)/(cur_ticks - wticks_1s));_DBG_(" samples in 1s.");
...
#if ERROR_LOG        
        clock_time_t cur_ticks = clock_time();
        _DBD32(cur_ticks);_DBG_(": Play so slow!!!");
#endif        
    }
...
uint32_t AudioBuf_Read(AUDIO_BUFFER *buff, uint8_t* data, uint32_t size)
{
    int n_rbytes = __AUDIO_BUFF_NUM(buff);
    
    if(n_rbytes > size)
        n_rbytes = size;
    memcpy(data, &buff->buffer[buff->head], n_rbytes);
    __BUF_INCR_NUM(buff->head, n_rbytes, buff->buffer_size);
...
Status Play_Setup(void)
{
    ...
    _DBG("[DEBUG]Bitrate = ");_DBD32(mp3_frame_info.bitrate);_DBG_("");
    ...
    Uda1380_Init(100000, mp3_frame_info.samprate, mp3_frame_info.nChans);
    Uda1380_MasterVolCtrl(0xB4,0xB4);
    ...
    do
    {
        if(start)
        {
            if((play_int_stat == SET) ||
                (AUDIO_BUFF_SIZE/2 > __AUDIO_BUFF_NUM(&audio_buffer)))
                break;        
            play_int_stat = SET;
        ...
        {        
            clock_time_t cur_ticks = clock_time();
            _DBD32(cur_ticks);_DBG_(": Play so fast!!!");
        }
#endif    
        }
    }while(0);
	...
	if (GPDMA_IntGetStatus(GPDMA_STAT_INT, 0)){ //check interrupt status on channel 0
		...
		if(GPDMA_IntGetStatus(GPDMA_STAT_INTTC, 0)){
...
#if DEBUG_DATA_READ   
            {
                ...
                {
                    _DBG("Read: ");_DBD32(((uint64_t)rsamples_cnt*CLOCK_CONF_SECOND)/(cur_ticks - rticks_1s ));_DBG_("samples/s");
...
void Decoder_FillInputBuffer(uint8_t* data, uint32_t size)
{
    ...
    // Copy frame data into input buffer
    if((input_frame_size + size) >= DECODER_IN_BUFF_SIZE)
    {
#if ERROR_LOG        
        _DBG_("Input Decoder Buffer Underflow!!!");
#endif       
...
int Decoder_Decode(void)
{
    ...
    int sync_word = 0;
    int            decode_in_size;
    unsigned char* decode_in_buf = (unsigned char*)DECODER_IN_BUFF_START_ADDR;
    unsigned char* decode_out_buf = (unsigned char*)DECODER_OUT_BUFF_START_ADDR;   
    int frame_num = 0;
#if DEBUG_DECODE_TIME
    clock_time_t dstart_ticks = clock_time();
    clock_time_t dend_ticks;
    ...
    while((input_frame_size >= IN_FRAME_MIN_SIZE) && (!exit_flag)) { 
        decode_in_buf = (unsigned char*)DECODER_IN_BUFF_START_ADDR;
        // Find the start of actual MP3 data
        sync_word = MP3FindSyncWord(decode_in_buf, input_frame_size);
        ...
        decode_in_size = input_frame_size;
        decode_in_buf += sync_word;
#if TRACE_LOG        
        _DBG("Found sync word at ");_DBD32(sync_word);_DBG_("");
#endif    
        
        err = MP3Decode(mp3_decoder, (unsigned char**)&decode_in_buf, 
                        &decode_in_size, (short *)decode_out_buf, 0);      
        if(err)
        {
            switch(err)
            {
...
#endif                
                    decode_in_size = input_frame_size - sync_word;
                    decode_in_buf = (unsigned char*)(DECODER_IN_BUFF_START_ADDR + sync_word);
                    exit_flag = 1;
                    break;
...
#endif                
                    if(decode_in_size == (input_frame_size - sync_word))
                    {
                        decode_in_size--;
                        decode_in_buf++;
                    }
        ...
        {  
            ...
            AudioBuf_Write(&audio_buffer, (uint8_t*)decode_out_buf, audio_frame_size);
            samples += mp3_frame_info.outputSamps/mp3_frame_info.nChans;
            frame_num++;
            //exit_flag = 1;
        }
        if((decode_in_size > 0) && (decode_in_size != input_frame_size))
        {
            memmove((char*)DECODER_IN_BUFF_START_ADDR, 
                    (char*)decode_in_buf, 
                    decode_in_size);
            input_frame_size = decode_in_size;
         }
         else if(decode_in_size == 0)
         {
             input_frame_size = 0;
         }
   }    
   ...
   {
#if TRACE_LOG        
       _DBD32(frame_num);_DBG_(" frames decoded.");
#endif       
       audio_frame_num = frame_num;
       Play_Control(1);
   }
   return samples;
}
