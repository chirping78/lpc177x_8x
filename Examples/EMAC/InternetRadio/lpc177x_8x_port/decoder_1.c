...
#endif    
    if((input_frame_size) && (read_ptr != DECODER_IN_BUFF_START_ADDR))
    {
...
int Decoder_Decode(void)
{
    ...
    int sync_word = 0;
    int            decode_in_size;
    unsigned char* decode_in_buf = (unsigned char*)read_ptr;
    ...
    while((input_frame_size >= IN_FRAME_MIN_SIZE) && (!exit_flag)) { 
        ...
        decode_in_buf = (unsigned char*)read_ptr;
...
#endif                
                    if(decode_in_size == input_frame_size)
                    {
                        input_frame_size--;
                        read_ptr++;
                    }
...
#endif                
                    read_ptr = (uint32_t)decode_in_buf;
