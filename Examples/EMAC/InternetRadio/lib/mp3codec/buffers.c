...
MP3DecInfo *AllocateBuffers(void)
{
	...
	s_mp3DecInfo.HuffmanInfoPS =     (void *)&s_hi;
	s_mp3DecInfo.DequantInfoPS =     (void *)&s_di;
	s_mp3DecInfo.IMDCTInfoPS =       (void *)&s_mi;
	s_mp3DecInfo.SubbandInfoPS =     (void *)&s_sbi;

		/* important to do this - DSP primitives assume a bunch of state variables are 0 on first use */
	ClearBuffer(&s_fh,  sizeof(FrameHeader));
	ClearBuffer(&s_si,  sizeof(SideInfo));
	ClearBuffer(&s_sfi, sizeof(ScaleFactorInfo));
	ClearBuffer(&s_hi,  sizeof(HuffmanInfo));
	ClearBuffer(&s_di,  sizeof(DequantInfo));
	ClearBuffer(&s_mi,  sizeof(IMDCTInfo));
	ClearBuffer(&s_sbi, sizeof(SubbandInfo));
